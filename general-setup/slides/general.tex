...
\begin{frame}{The instructions file}
  \begin{itemize}
    \item Agentic tools read a Markdown file at every session start ---
          persistent project memory.
    \item Codex ($+\sim$20 tools) call it \texttt{AGENTS.md}; Claude Code calls
          it \texttt{CLAUDE.md} (and also reads \texttt{AGENTS.md}).
    \item Open standard since Aug 2025 (Linux Foundation); 60{,}000+ repos.
          Just Markdown, no required fields.
    \item Like a ``README for the agent'': what you'd tell a new RA, written once.
  \end{itemize}
  \vfill
  \href{https://agents.md/}{\beamergotobutton{agents.md}}\quad
...
\begin{frame}{Using it well}
  \begin{itemize}
    ...
    \item Both run in a sandbox and can execute code + edit files ---
          power and risk for reproducibility.
  \end{itemize}
  \vfill
...
\begin{frame}{Skills: what they are}
  \begin{itemize}
    \item A folder with a \texttt{SKILL.md} (YAML \texttt{name} + \texttt{description},
          then Markdown instructions); can bundle scripts + resources.
    \item Like an onboarding guide for a repeatable task
          (e.g.\ ``format regression tables'').
    \item \textbf{Progressive disclosure}: metadata always loaded ($\sim$100 tokens);
          instructions load on trigger; bundled files/scripts load only when used.
    \item The \texttt{description} decides when Claude fires the skill --- say
          \emph{what} it does and \emph{when} to use it.
  \end{itemize}
  \vfill
